...
from zhenxun.plugins.welcome_new import file_to_bytes
...
async def download_b_file(url: str, file_path: Path):
    ...
    proxies = f"http://{proxy_setting}" if proxy_setting else None
...
async def send_video_final(bot: Bot, event: Event, video_path: Path):
    ...
    video_bytes = file_to_bytes(Path(path_str))
    video_segment = V11MessageSegment.video(video_bytes)
...
async def extract_bilibili_url_from_miniprogram(raw_str: str) -> str | None:
    """从小程序消息提取B站URL"""
...
async def check_video_duration(
    duration_seconds: float, event: Event, matcher: AlconnaMatcher | None = None
) -> bool:
    """检查视频时长是否超过限制"""
...
def get_bangumi_info_sync(url_info: dict[str, Any]) -> dict[str, Any] | None:
    """获取番剧信息(同步)"""
    api_url = None
...
async def _perform_bangumi_download(
    ...
    matcher: AlconnaMatcher | None = None,
):
    """下载并发送番剧视频"""
    ...
    try:
        if "dash" in play_info:
            ...
            a_stream_path = (
                PLUGIN_TEMP_DIR / f"bili_video_ep{ep_id}_audio.m4s"
                if audio_url
                else None
            )
...
async def _perform_video_download(
    ...
    if cached_file:
        ...
        try:
            ...
            path_str = path_str.replace("\\", "/")
            file_uri = "file:///" + path_str
            byte_file = file_to_bytes(Path(path_str))
            video_segment = V11MessageSegment.video(byte_file)
            logger.debug(
    ...
    try:
        ...
        selected_video_stream: dict[str, Any] | None = None
        selected_audio_stream: dict[str, Any] | None = None
        video_url = None
        audio_url = None
        use_copy_codec = False
        ...
        if merge_success:
            ...
            if downloaded_file_path.exists():
                try:
                    ...
                    path_str = path_str.replace("\\", "/")
                    file_uri = file_to_bytes(Path(path_str))
...
async def handle_bili_download(
    ...
    url_info_dict: dict[str, Any] | None = None
